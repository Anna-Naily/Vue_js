...
<div>
    ...
    <h2 v-else>{{ result }}</h2>
    <button v-for="item in operators" :key="item" @click="calculation(item)">{{ item.toUpperCase() }}</button><br>
    ...
    <div class="keyboard" v-show="keyboardVisible">
        <div class="buttons"> <button v-for="item in keyboard" :key="item" @click="numberInput(item)">{{ item }}</button></div>
        <div class="radio">
